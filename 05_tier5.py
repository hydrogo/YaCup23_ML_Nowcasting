...
MODEL = "Unet"
BACKBONE = "vgg16" # important change
LEADTIME = 1
EPOCHS = 20 # advice results of tier 3 and 4!!!
BATCH = 32 # important change: decrease batch to fit higher memory requirements

keys = np.load("./keys.npy", allow_pickle=True).item() # important change: updated training keys
...
if LEADTIME == 1:
    mkey = 5 # important change: from 3 to 5 as we have different number of input frames
elif LEADTIME == 12:
    mkey = 16
...
class DLSequence(Sequence):
    ...
    def __init__(self, index_instance, data_path=data_path, batch_size=16, lookback=4, shuffle=False): # important change: default values of lookback and batch
...
input_frames = 4 # important change: from 2 to 4
...
# Set-up Callbacks
checkpoint_filepath = f'./models/{MODEL}_{BACKBONE}_{LEADTIME}_{TRAINSET}_tier5_repr.keras'
...
reduceLR_callback = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', 
                                                         min_delta=0.0001,
                                                         mode="min", 
                                                         patience=2, # advise changes regarding results of tier3 and tier4
                                                         factor=0.1)
...
print(f"Tier 5, trainset: {TRAINSET}.")
